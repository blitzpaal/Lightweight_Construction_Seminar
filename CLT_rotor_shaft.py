...
# Laminate stack
stack = np.array([[0,t_ply,0],[45,t_ply,0],[-45,t_ply,0],[90,t_ply,0],[90,t_ply,0],[-45,t_ply,0],[45,t_ply,0],[0,t_ply,0]]) # [angle, thickness, height] Laminate buildup with orientation (in degree), thickness (in mm) and z-position (in mm, calculated later) of each layer
stack[:,1] = np.deg2rad(stack[:,1]) # transform ply angle from degree to radian
...
for i in range(stack.shape[0]):
    if i>0:
        stack[i,2] = -z_ref+stack[i-1,2]

    T_s = np.array([[np.cos(stack[i,0])**2, np.sin(stack[i,0])**2, -2*np.sin(stack[i,0])*np.cos(stack[i,0])], [np.sin(stack[i,0])**2, np.cos(stack[i,0])**2, 2*np.sin(stack[i,0])*np.cos(stack[i,0])],
    [np.sin(stack[i,0])*np.cos(stack[i,0]), -np.sin(stack[i,0])*np.cos(stack[i,0]), np.cos(stack[i,0])**2-np.sin(stack[i,0])**2]])

    T_e = np.array([[np.cos(stack[i,0])**2, np.sin(stack[i,0])**2, np.sin(stack[i,0])*np.cos(stack[i,0])], [np.sin(stack[i,0])**2, np.cos(stack[i,0])**2, -np.sin(stack[i,0])*np.cos(stack[i,0])],
    [-2*np.sin(stack[i,0])*np.cos(stack[i,0]), 2*np.sin(stack[i,0])*np.cos(stack[i,0]), np.cos(stack[i,0])**2-np.sin(stack[i,0])**2]])

    Q_trans[:,:,i] = np.transpose(T_s) * Q_0 * T_e

# ABD Matrix calculation
A = np.sum(Q_trans*stack[:,1],2)
B = np.sum(Q_trans*stack[:,1]*(stack[:,2]-stack[:,1]/2),2)
D = np.sum(Q_trans*(stack[:,1]**3/12+stack[:,0]*(stack[:,2]-stack[:,1]/2)**2),2)